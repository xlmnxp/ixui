...
function SpiceMsgcMousePosition(sc, e)
{
    ...
    if (e)
    {
        // Map CSS-scaled canvas coordinates back to framebuffer coordinates.
        var target = e.target;
        var rect = target.getBoundingClientRect();
        var scaleX = rect.width > 0 && target.width ? target.width / rect.width : 1;
        var scaleY = rect.height > 0 && target.height ? target.height / rect.height : 1;
        this.x = Math.round(e.offsetX * scaleX);
        this.y = Math.round(e.offsetY * scaleY);
